...
impl <'a> Parser<'a>{
    pub(crate) fn parse_create_statement(&mut self)->(AstNode,Option<triadic_error::Compiler>){
        ...
        return (AstNode::Nothing,Some(triadic_error::Compiler::CREATE))
    }
    fn parse_create_database_statement(&mut self)->(AstNode,Option<triadic_error::Compiler>){
            ...
            self.tokens.get(self.current_token)
        {
            ...
            if let Some(Token::Punctuation(';'))=self.tokens.get(self.current_token){
                ...
                return (AstNode::CreateDatabaseStatement(db_name.clone()),None)
            }else {
                return (AstNode::Nothing,Some(triadic_error::Compiler::CreateDatabaseIdentifier))
            }
        }
        return (AstNode::Nothing,Some(triadic_error::Compiler::CreateDatabase))
    }
}